/// <copyright file="CoreFrameworkMonitorWindow.cs">Copyright (c) 2015 All Rights Reserved</copyright>
...
namespace CoreFramework.Editor {
    ...
    public class CoreFrameworkMonitorWindow : EditorWindow {

        public const string MonitorHelpUrl = "http://unityCoreFramework.com/monitors/";
        ...
        private static CoreFrameworkMonitorWindow instance;
        ...
        private static void Init() {
            if (instance) { Deinit(); }
            hasInit = true;
            instance = EditorWindow.GetWindow(typeof(CoreFrameworkMonitorWindow)) as CoreFrameworkMonitorWindow;
        ...
        private void DrawRelationWindowSelectionButton(string buttonText, IWindowRenderer windowRenderer) {
            if (currentWindowRenderer == windowRenderer) { 
                GUI.color = CoreFrameworkEditorStyles.MainColor;
            } else {
                GUI.color = Color.white;
            }
        ...
        private void RenderRelationsWindow(int id) {
            ...
            if (MiniMapEnabled) {
                RenderingHelper.RenderRect(MiniMapWindowRect, new Color(.5f, .5f, .5f, .3f * (CoreFrameworkMonitorWindow.IsHoveringMiniMap ? 1.0f : CoreFrameworkEditorStyles.MiniMapMouseOutAlpha)));
                ...
                RenderingHelper.RenderRect(new Rect(MiniMapWindowRect.x - (WindowOffset.x - WindowPadding.x) * MiniMapScaleFactor,
                                          ...
                                          windowRect.height * MiniMapScaleFactor), new Color(1.0f, 1.0f, 1.0f, .2f * (CoreFrameworkMonitorWindow.IsHoveringMiniMap ? 1.0f : CoreFrameworkEditorStyles.MiniMapMouseOutAlpha)), MiniMapWindowRect);
            } else {
