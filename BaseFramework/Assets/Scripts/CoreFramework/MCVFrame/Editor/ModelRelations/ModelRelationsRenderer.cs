    ...
    public class ModelRelationsRenderer : IWindowRenderer {
        ...
        public void ShowContextMenu() {
            ...
            menu.AddItem(new GUIContent("What is this?"), false, delegate() {
                Application.OpenURL(CoreFrameworkMonitorWindow.MonitorHelpUrl);
            });
        ...
        public void Render() {
            if (modelRelationsWidgets.Count > 0) {
                CoreFrameworkEditorStyles.SetLabelStyle(CoreFrameworkEditorStyles.LabelStyle.ModelRelationsTitle);
                GUI.Label(new Rect(CoreFrameworkMonitorWindow.WindowOffset.x, CoreFrameworkMonitorWindow.WindowOffset.y, ModelRelationsWidget.Width, 30), "Actor");
                GUI.Label(new Rect(CoreFrameworkMonitorWindow.WindowOffset.x + ModelRelationsWidget.WidgetMarginHor, CoreFrameworkMonitorWindow.WindowOffset.y, ModelRelationsWidget.Width, 30), "Model");
                GUI.Label(new Rect(CoreFrameworkMonitorWindow.WindowOffset.x + 2.0f * ModelRelationsWidget.WidgetMarginHor, CoreFrameworkMonitorWindow.WindowOffset.y, ModelRelationsWidget.Width, 30), "Handler");
                CoreFrameworkEditorStyles.ResetLabelStyle();

                foreach (ModelRelationsWidget widget in modelRelationsWidgets) {
                    widget.Render();
                }
            } else {
                CoreFrameworkEditorStyles.SetLabelStyle(CoreFrameworkEditorStyles.LabelStyle.WarningMessage);
                GUI.color = CoreFrameworkEditorStyles.NoContentColor;
