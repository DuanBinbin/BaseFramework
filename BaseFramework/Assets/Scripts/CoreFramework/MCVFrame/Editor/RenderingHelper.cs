...
namespace CoreFramework.Editor {
    ...
    public class RenderingHelper {
        ...
        public static void RenderDotInMonitorWindow(Vector2 point, Color color, float width, float height) {
            RenderDot(point + CoreFrameworkMonitorWindow.WindowOffset, color, width, height);
        }
        ...
        public static void RenderRectInMonitorWindow(Rect rect, Color color) {
            RenderRect(new Rect(rect.x + CoreFrameworkMonitorWindow.WindowOffset.x, rect.y + CoreFrameworkMonitorWindow.WindowOffset.y, rect.width, rect.height), color);
        }
        ...
        public static void RenderLineInMiniMap(Vector2 pointA, Vector2 pointB, Color color, int width) {
            color.a *= (CoreFrameworkMonitorWindow.IsHoveringMiniMap ? 1.0f : CoreFrameworkEditorStyles.MiniMapMouseOutAlpha);
            Vector2 miniMapPos = CoreFrameworkMonitorWindow.MiniMapWindowRect.position + CoreFrameworkMonitorWindow.WindowPadding * CoreFrameworkMonitorWindow.MiniMapScaleFactor;
            float miniMapScale = CoreFrameworkMonitorWindow.MiniMapScaleFactor;
        ...
        public static void RenderLineInMonitorWindow(Vector2 pointA, Vector2 pointB, Color color, int width) {
            RenderLine(pointA + CoreFrameworkMonitorWindow.WindowOffset, pointB + CoreFrameworkMonitorWindow.WindowOffset, color, width);
        }
